...
@pytest.fixture
def empty_grid():
    return Grid(np.matrix(np.full((15, 15), '', dtype=str)))


@pytest.fixture
def mock_tree():
    return MockTree(['test', 'tout', 'atout', 'soir', 'rat', 'ta', 'or'])
...
def test_is_word_in_bounds(word_placer):
    ...
    assert not word_placer._is_word_in_bounds((7, 7), 'verylongwordthatdoesntfit', Direction.HORIZONTAL)
...
def test_check_first_word_placement(word_placer):
    assert word_placer._check_first_word_placement((7, 7), 'test', Direction.HORIZONTAL)['state']
    assert word_placer._check_first_word_placement((7, 7), 'test', Direction.VERTICAL)['state']
    assert not word_placer._check_first_word_placement((0, 0), 'test', Direction.HORIZONTAL)['state']
    assert not word_placer._check_first_word_placement((14, 14), 'test', Direction.VERTICAL)['state']
...
def test_check_perpendicular_word(word_placer):
    word_placer.grid[7, 7:10] = list('tou')
    assert word_placer._check_perpendicular_word((8, 7), 'a', Direction.VERTICAL)['state']
    assert word_placer._check_perpendicular_word((6, 8), 'x', Direction.VERTICAL)['state']
    assert word_placer._check_perpendicular_word((7, 10), 't', Direction.VERTICAL)['state']
    assert not word_placer._check_perpendicular_word((7, 10), 'x', Direction.VERTICAL)['state']


def test_check_word_placement(word_placer):
    word_placer.grid[7, 7:11] = list('tout')
    assert word_placer._check_word_placement((6, 8), 'atout', Direction.VERTICAL)['state']
    assert not word_placer._check_word_placement((6, 8), 'axout', Direction.VERTICAL)['state']
...
def test_is_word_placable(word_placer):
    ...
    # Valid placements
    assert word_placer.is_word_placable((6, 8), 'tout', Direction.VERTICAL)['state']
    assert word_placer.is_word_placable((7, 11), 'or', Direction.HORIZONTAL)['state']

    # Invalid placements
    assert not word_placer.is_word_placable((7, 7), 'test', Direction.HORIZONTAL)['state']  # Overlapping
    assert not word_placer.is_word_placable((0, 0), 'test', Direction.HORIZONTAL)['state']  # Not touching
    assert not word_placer.is_word_placable((6, 8), 'invalid', Direction.VERTICAL)['state']  # Invalid word


def test_is_word_placable_edge_cases(word_placer):
    # Word too long
    assert not word_placer.is_word_placable((7, 7), 'a' * 16, Direction.HORIZONTAL)['state']

    # Word at edge of grid
    assert word_placer.is_word_placable((14, 7), 'test', Direction.HORIZONTAL)['state']
    assert not word_placer.is_word_placable((14, 7), 'test', Direction.VERTICAL)['state']
